    ...
    return (
      <div className="brandBox">
        ...
        <div className="brandWrap">
          {
            brandList.map(item => (
              <Link to={`/brandDetail/${item.id}`} className="brandItem" key={item.id}>
    ...
    <div className="topGoodsBox">
      ...
      <div className="topGoodsWrap">
        <Carousel dots={false} infinite  slideWidth={0.85} cellSpacing={10}>
          {topicList.map(item => (
           <Link to={`/topicDetail/${item.id}`} key={item.id} className="topGoodsItem">
             <ImgLazyLoad
               offSetTop={44}
...
class App extends Component {
  ...
  render() {
    const {banner, newGoodsList, channel, hotGoodsList, brandList, topicList, categoryList } = this.props
    return (
      <Fragment>
